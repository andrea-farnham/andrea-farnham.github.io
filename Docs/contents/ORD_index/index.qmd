...
The *img src* usually has the path to the images relative to the current path e.g., ".\Images" if they are in subfolder of the current working directory (in R you can find it with `getwd()`). The current path "../../Images" indicates they are in a folder named 'Images' two folders above current folder. This is related to how our Continuous integration (CI/CD) pipeline is set. There are many other possible configurations. Beware your local relative paths may not function the same if you use CI/CD and Gitlab pages. Find more information about CI/CD is in our Tutorial section. 
:::
